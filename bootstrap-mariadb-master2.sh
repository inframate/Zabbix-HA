...
cp /CONFIGs/$CONFIG/master2.cnf /etc/my.cnf.d/
...
mysql -uroot -pdevops -e 'SHOW MASTER STATUS\g' > /CONFIGs/$CONFIG/master2_status
#give root remote access from other cluster members
mysql -uroot -pdevops -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'mariadb-master1.domain.com' IDENTIFIED BY 'devops' WITH GRANT OPTION;"
